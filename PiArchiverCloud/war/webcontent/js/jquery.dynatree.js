...
//          logMsg("isHidden:%s", isHidden);
	...
	sortChildren: function(cmp, deep) {
		...
		if(!cmp){
			if(this.data.sort===0)
				this.data.sort = 1;//asc
			else if(this.data.sort===1)
				this.data.sort = -1;//des
			else
				this.data.sort = 1;//asc
		}
		//phan end
...
//          return a.data.title === b.data.title ? 0 : a.data.title > b.data.title ? 1 : -1;
			...
			return x === y ? 0 : x > y ? sort : 0-sort;//phan add
			};
		cl.sort(cmp);
		if( deep ){
			for(var i=0, l=cl.length; i<l; i++){
				if( cl[i].childList ){
					cl[i].data.sort = sort;//phan add
...
//          this.isRead = false;
	...
	//phan add start
	setKey: function(key) {
		this.fromDict({key: key});
	},
	setSort: function(sort) {
		this.fromDict({sort: sort});
	},
	getSort: function() {	
...
$.ui.dynatree.nodedatadefaults = {	
	...
	key: 0, // May be used with activate(), select(), find(), ...
...
//  disabled: false,
	...
	//phan add start
	sort:0,
	newid:0//0 means not new, newid >0 is new in order
	//phan add end
};
